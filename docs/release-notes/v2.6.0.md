...
**Jellyfin 12 is supported.** This release ships a second build so the plugin loads and runs on Jellyfin 12 (which moved to .NET 10), while the existing build keeps serving Jellyfin 10.11.x exactly as before. Both live in the one plugin catalog under the same entry — your server downloads the build that matches its version, so there is nothing to pick and existing installs update themselves. It also fixes the admin step-up prompt on both the settings save path and the OIDC provider form. Drop-in from any 2.5.x — no config or data migration.
...
Everything was verified end to end on a real Jellyfin 12 server: sign-in, refresh persistence, OIDC/SSO, TOTP, email OTP, recovery codes, passkeys, app passwords, admin step-up, the PDF recovery-code export, and the admin dashboard page.
...
- Completes the header work from v2.5.22 (#174/#180): the plugin's pages and endpoints already accept Jellyfin's current `Authorization: MediaBrowser Token` header, which 12 requires once legacy authorization is disabled.
...
- **GeoIP diagnostics (#51)** were already fixed in v2.5.7 (the availability check now loads the databases on first touch instead of showing "Fail" until a lookup happened). Noted here for anyone still tracking it from an older build.

## Upgrade notes

In-place upgrade from any 2.5.x — no schema or configuration migration. 488 tests pass. Sigstore-signed and SLSA build-provenance attested. Both the 10.11 (.NET 9) and 12 (.NET 10) packages are published for this version; your server selects the right one automatically.
